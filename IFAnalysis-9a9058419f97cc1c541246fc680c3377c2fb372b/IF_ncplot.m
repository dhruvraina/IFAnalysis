...
%             plotflag.imageFormat = 'fig';
    ...
    case ('boxplot')
        ...
        maxlen   = max(cellfun(@(x) length(x), resvec));
        treatNum = length(resvec);
        boxDat   = nan(maxlen, treatNum);
        
        for aa = 1:treatNum
           tvec                      = resvec{aa}(:,1); 
           boxDat(1:length(tvec),aa) = tvec; 
           spreadDat{aa}             = resvec{aa}(:,1); 
           clear tvec
        end
         
        fig2 = figure, plotSpread(spreadDat, 'distributionColors', colors(1:treatNum,:))
        hold on
        boxplot(boxDat)
